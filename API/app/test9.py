...
class Test9:
    ...
    def detect_face_with_mtcnn(self, image: str):
    ...
    def detect_face_with_opencv(self, image: str):
        ...
        scaleFactor=data[0]['scaleFactor']
        minNeighbors=data[0]['minNeighbors']
    ...
    def preprocess_image(self, image_path: str):
        ...
        if len(faces) > 0:
            ...
            face_image_mtcnn_resized = cv2.resize(face_image_mtcnn, (image_size, image_size))
        else:
        ...
        face_cascade = cv2.CascadeClassifier(cv2.data.haarcascades + 'haarcascade_frontalface_default.xml')
        faces_opencv = face_cascade.detectMultiScale(image_gray, scaleFactor, minNeighbors)
        if len(faces_opencv) > 0:
            ...
            # Resize ảnh khuôn mặt về kích thước 160x160
            face_image_opencv_resized = cv2.resize(face_image_opencv, (image_size, image_size))
        else:
            # Nếu không tìm thấy khuôn mặt, trả về None
